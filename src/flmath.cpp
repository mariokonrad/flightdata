...
double central_spherical_angle(const position_t & p0, const position_t & p1)
{
	// not used because of potential numerical problems:
	// return acos(sin(p1.lat) * sin(p0.lat) + cos(p1.lat) * cos(p0.lat) * cos(p1.lon - p0.lon));

	return atan(
		sqrt(
			sqr(cos(p1.lat) * sin(p1.lon - p0.lon))
...
double distance_sphere(const position_t & p0, const position_t & p1)
{
	return EARTH_RADIUS * central_spherical_angle(p0, p1);
}

double distance_ellipsoid_vincenty(const position_t & p0, const position_t & p1, double & alpha1, double & alpha2) // {{{
{
...
position_t point_ellipsoid_vincenty(const position_t & p0, double s, double alpha1, double & alpha2) // {{{
{
	...
	const double A = 1.0 + u_sqr / 16384.0 * (4096.0 + u_sqr * (-768.0 + u_sqr * (320.0 - 175.0 * u_sqr))); // eq 3

	const double B = u_sqr / 1024.0 * (256.0 + u_sqr * (-128.0 + u_sqr * (74.0 - 47.0 * u_sqr))); // eq 4
...
double distance_ellipsoid_lambert(const position_t & p0, const position_t & p1) // {{{
{
	const double r = 1.0 / EARTH_FLATTENING;

	position_t t1 = { atan((r - 1.0) / r) * tan(p0.lat), p0.lon };
	position_t t2 = { atan((r - 1.0) / r) * tan(p1.lat), p1.lon };

	double sigma = central_spherical_angle(t1, t2);

	double P = (t1.lat + t2.lat) / 2.0;

	double Q = (t2.lat - t1.lat) / 2.0;

	double X = (sigma - sin(sigma)) * (sqr(sin(P)) * sqr(cos(Q))) / sqr(cos(sigma / 2.0));

	double Y = (sigma + sin(sigma)) * (sqr(cos(P)) * sqr(sin(Q))) / sqr(sin(sigma / 2.0));
